 ...
 * 2. リッチメニューB「写真を追加する」→ 撮影ガイド（まずは3枚方式）＋カメラボタンを返信
...
const DEFAULT_FEES = {
  ...
  'かほく市': '1,000円', '津幡町': '1,000円', '内灘町': '1,500円',
  '宝達志水町': '2,000円', '羽咋市': '2,000円', '野々市市': '2,000円', '野々市': '2,000円',
  '能美市': '2,500円', '川北町': '3,000円', '志賀町': '2,500円', '中能登町': '2,500円',
...
function replyCityFee(replyToken, hit, event, intent) {
  ...
  if (intent === 'buy') {
    ...
    lines = [
      ...
      '',
      'そのエリアは かほく市役所からのGoogleマップ走行距離で変わるため、出張費は町名を伺って個別にご案内します（買取のご依頼なら出張費はかかりません）。',
...
function photoGuideMessage(withIntake) {
  // withIntake=true のときだけ「市町名・防犯登録」を尋ねる。買取査定の申込初回でのみ true。
  // リッチメニューB「写真を追加する」は申込済みの導線なので false（同じことを再質問しない）。
...
function replyKaitoriApply(replyToken, userId) {
  ...
  reply(replyToken, [{ type: 'text', text: ack }, photoGuideMessage(true)]);
...
function replyInquiry(event) {
  ...
  const text = [
    ...
    '',
    '※ 買取・引取のご相談は、下の☰メニューから「買取を申し込む」または「処分・引取」をタップすると案内が始まります。',
  ].join('\n');
  reply(event.replyToken, [{ type: 'text', text: text }]);
  const userId = event.source && event.source.userId;
...
function handlePostback_(event, userId) {
  ...
  if (isManualMode_(st)) {
    ...
    // nav（やめる・戻る・やり直す・相談）は営業応答ではないので手動対応中でも通す（モードは維持）
  }

...
EBIKE_NOTE = ['1・2 は自転車ぜんぶ（右・左）。7・8 と 9・10 は右と左から1枚ずつ。',
              '13・14 はバッテリーの鍵と型番（メーカーで位置が違います）。15 は充電器に載せた状態。']
...
def photo_map(src_dir, src_id, pos, title, notes):
    ...
    for i, t in enumerate(notes):
        d.text((24, TOP + H + 22 + i * 44), t, font=mm.font(24, bold=False), fill=SUB)
    return im

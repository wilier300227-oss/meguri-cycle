...
FRAMES = {
    ...
    'R01': (310, 770, 730, 980),     # 車体の品番シール（実写真）。枠はいちばん下の型番ラベル（上は安全マーク、真ん中はメーカーロゴ。2026-09-22 オーナー指摘）
...
NORMAL = {
    1: ('3337', '右から全体', '', False), 2: ('3338', '左から全体', '', False), 3: ('R01', '品番シール（いちばん下の型番）', '', False),
...
EBIKE = {
    1: ('3343', '右から全体', '', False), 2: ('3338', '左から全体', '', False), 3: ('R01', '品番シール（いちばん下の型番）', '', False),
...
_NL = chr(10)
SET_NOTES = {
    ('normal', 1): _NL.join(['品番シールは、型番が', '読めるように近づけて。', '', 'シールがない場合は、', 'ロゴや車種名が分かる', 'ものでも大丈夫です。']),
    ('ebike', 1): _NL.join(['品番シールは、型番が', '読めるように近づけて。', '', 'シールがない場合は、', 'ロゴや車種名が分かる', 'ものでも大丈夫です。']),
    ('ebike', 2): _NL.join(['手元スイッチは', '電源を入れて、', '数字が読めるように。']),
    ('ebike', 5): _NL.join(['型番シールは、', '文字が読めるように', '近づけて。', '', '充電器は、ランプが', '点いている瞬間を。']),
}

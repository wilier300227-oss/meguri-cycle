...
- **PDF/PNGの描画崩れ（拡大・枠切れ）は「Redmi Pad SE 実機＋オフライン」限定で、PC・オンラインでは再現しない。** 過去③-A-3/7/9 で修正を繰り返したが、2026-08-25 に再発（毎回発生）。
  → **運用判断（2026-08-25）：タブレットは基本オンラインで使用し、オフラインでのPDF/PNG生成は非保証扱い。** 描画（`renderSheet()`/`waitForImages()`）を触った場合の確認は PC＋オンラインで足りる（オフライン検証は必須としない）。オフライン時の運用注意（複数台はオフライン中に確定しない等）は `kaitori/HANDOFF.md` ⑤ を参照。
  - LAN検証が必要な場合は、このPCで `python -m http.server`（リポジトリ直下。8765が別サーバに占有されていたら別ポート）を立て、タブレットから `http://<PCのLAN IP>:<ポート>/kaitori/` を開く。一度開くとSWがキャッシュするため、再検証時はSW解除＋キャッシュ削除が必要。
- 出力パネル（PDF/PNG保存画面）は**署名確定→確認ダイアログOK後（`doConfirm`）だけ**に表示する。起動時など他経路で先に出さない（お客さまの誤タップ防止）。

## 調査資料の扱い

- 調査や前提知識が必要なタスクでは、まず `docs/research/` を確認する
- 各ファイルの frontmatter の `date` を必ず見る。3ヶ月以上前のものは古い可能性があるとして扱い、判断の根幹に関わる部分だけ WebSearch で裏取りする
- 実装中の「このライブラリ/APIの現行仕様は？」程度の確認は、内蔵の WebSearch で完結させてよい
- 一方で、複数の観点から掘る必要がある調査（市場調査・法令確認・技術選定の比較検討など）は、自前で検索を重ねず、ユーザーに
  「claude.ai の Research か Perplexity で調べて docs/research/ に置いてください」
  と依頼すること
- 新しい調査資料を置くときは `docs/research/_template.md` を雛形にし、命名は `YYYY-MM-DD-テーマ名.md`（日付は調査実施日）とする。更新時は新規ファイルを作り、古いものは残す
